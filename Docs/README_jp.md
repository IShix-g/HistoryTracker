...
ダイアログはスクリプトで開きます。必要無くなったタイミングで`Hist.Release()`で解放する事も可能ですが、軽いので問題になる可能性は低いと思います。
...
## アセット読み込みに使用するAPI
`Resources.Load` - 実機でUIを読み込む為に利用。プラグインが有効なときだけゲームのビルド結果に含まれます。プラグインが無効だとアセットは省かれ、アプリに含まれません。
