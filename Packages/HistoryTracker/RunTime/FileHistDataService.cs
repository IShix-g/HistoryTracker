...
{
    internal abstract class FileHistDataService : IHistDataService
    {
        readonly string _rootDir;
        readonly string _dataPath;
        HistRecords _records;

        protected FileHistDataService(string rootDir, string dataPath)
        {
            _rootDir = rootDir;
            _dataPath = dataPath;
            LoadRecords();
        ...
        public HistRecord Set(int recordId, IReadOnlyList<string> paths)
        {
        ...
        {
            var idString = record.Id.ToString();
            ...
            finally
            {
                ...
                {
                    Debug.LogWarning(
                        $"Apply でコピーが行われなかった record.Paths の要素があります。record.Id={record.Id}\n" +
                ...
                {
                    Debug.LogWarning(
                        $"Apply でコピーに使用されなかった paths の要素があります。record.Id={record.Id}\n" +
                        string.Join("\n", unusedPaths));
                }
